...
import { listArchiveHeists } from '../services/heists';
...
const IntelArchive = () => {
  ...
  // Fetch archived heists on mount
  useEffect(() => {
    fetchHeists();
  }, []);
  ...
  const fetchHeists = async () => {
    ...
    const result = await listArchiveHeists();
  ...
  const filteredHeists = heists.filter((heist) => {
    // Status filter (Expired or Aborted)
    ...
    return (
      ...
      heist.creator_username.toLowerCase().includes(query) ||
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto">
        {['All', 'Expired', 'Aborted'].map((status) => (
          <button
            key={status}
      ...
      <HeistGrid
        ...
        onAbort={() => {}} // No abort in archive
        currentUsername={user?.username}
        emptyMessage={
          searchQuery
            ? `No archived heists found matching "${searchQuery}".`
            : filterStatus === 'All'
            ? '📭 No archived heists yet.'
